...
class BaseDataMapper:
    ...
    def get_db_model(self, model: Base) -> Type[BaseDBAdapter.base]:
        db_model_cls: Optional[Type[BaseDBAdapter.base]] = getattr(self.db_models, model.__class__.__name__, None)
    ...
    def from_db_model(self, db_model: BaseDBAdapter.base) -> Base:
        model_cls: Optional[Type[Base]] = getattr(self.models, db_model.__class__.__name__, None)

        if not model_cls:
            raise TypeError(f"models not exist {db_model.__class__.__name__} model")

        return model_cls(**db_model.dict())

    async def create(self, model: Base) -> Optional[Base]:
    ...
    async def get(self, model: Base) -> Optional[Base]:
        db_model = self.get_db_model(model)
    ...
    async def get_list(self, model: Base) -> list[Base]:
        db_model = self.get_db_model(model)
    ...
    async def update(self, model: Base, **kwargs: Any) -> bool:
        db_model = self.get_db_model(model)

        async with self.adapter() as session:
            return await session.update(db_model, kwargs, **model.dict())

    async def get_or_create(self, model: Base) -> Optional[Base]:
        result = await self.get(model)
        if not result:
            result = await self.create(model)
        return result

    async def create_or_update(self, model: Base) -> Optional[Base]:
